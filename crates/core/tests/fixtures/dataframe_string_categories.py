"""Actual Qlib string/category append, including unused and ordered mixed dictionaries."""
import ast
...
dictionaries = {f"mixed_{i}": pd.Index(values, dtype=object)
                for i, values in enumerate(itertools.permutations(["a", 2, True, 2.5], 2))}
for name, values in dict(text=["a", "b"], numeric=[1, 2], boolean=[True, False], empty=[]).items():
    dictionaries[f"object_{name}"] = pd.Index(values, dtype=object)
for dtype in ["int64", "float64", "bool", "Int64", "Float64", "boolean"]:
    values = [True, False] if dtype in ["bool", "boolean"] else [1, 2]
    dictionaries[dtype] = pd.Index(values, dtype=dtype)
    dictionaries[dtype + "_empty"] = pd.Index([], dtype=dtype)
categories = {}
for (name, dictionary), ordered in itertools.product(dictionaries.items(), [False, True]):
    states = dict(empty=[], missing=[-1])
    if len(dictionary):
        states.update(first=[0, -1], last=[len(dictionary)-1])
