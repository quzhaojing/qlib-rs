"""Actual Qlib append with tuple/builtin mixed categorical dictionaries."""
import ast
...
categories = {}
for name, value in dict(text="a", integer=2, boolean=True, floating=2.5,
                        surrogate=chr(0xd800), tuple=(3,)).items():
    for reverse, ordered in itertools.product([False, True], [False, True]):
        values = [value, (1, 2)] if reverse else [(1, 2), value]
        dictionary = pd.Index(values, dtype=object, tupleize_cols=False)
